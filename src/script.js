...
$(document).ready(function () {
	...
	$('.art-head').draggable({
		...
		stop: function () {
			...
			const dropTo = window.innerHeight - Math.random() * 90 - 70 // Only drop between 70 and 160 from bottom
			...
			$(this).animate({ top: dropTo }, 500, function () {
				...
				// Get head's position strip units (px), and pad for aesthetics
				const headX = parseFloat(artHead.style.left.replace(/[^0-9.]/g, '')) + 20
				const headY = parseFloat(artHead.style.top.replace(/[^0-9.]/g, '')) - 5
